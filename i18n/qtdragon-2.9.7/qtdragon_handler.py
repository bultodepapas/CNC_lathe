...
""" %(  os.path.expanduser('~/linuxcnc'),
    ...
    def init_widgets(self):
        ...
        self.w.cmb_gcode_history.addItem(_translate("HandlerClass", "No File Loaded"))
        ...
        if self.w.jogincrements_linear.count():
            self.w.jogincrements_linear.setItemText(
                0, _translate("HandlerClass", "Continuous")
            )
        if self.w.jogincrements_angular.count():
            self.w.jogincrements_angular.setItemText(
                0, _translate("HandlerClass", "Continuous")
            )
        STATUS.connect('tool-info-changed', self._translate_tool_comment)
        self._translate_tool_comment(None, None)
    ...
    def _translate_tool_comment(self, obj, data):
        if STATUS.get_current_tool() == 0:
            self.w.tool_desc_status.setText(
                _translate("HandlerClass", "No Tool")
            )
...
#                    QtCore.Qt.Key_F3,QtCore.Qt.Key_F4,QtCore.Qt.Key_F5):
    ...
    def percent_loaded_changed(self, fraction):
        if fraction <0:
            ...
            self.w.progressBar.setFormat(
                '{}: {}%'.format(_translate("HandlerClass", "LOADING"), fraction)
            )
    ...
    def percent_done_changed(self, fraction):
        ...
        if fraction <0:
            ...
            self.w.progressBar.setFormat(
                '{}: {}%'.format(_translate("HandlerClass", "COMPLETE"), fraction)
            )
